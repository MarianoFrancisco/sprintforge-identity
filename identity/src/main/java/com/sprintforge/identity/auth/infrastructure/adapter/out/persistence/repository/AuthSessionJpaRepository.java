package com.sprintforge.identity.auth.infrastructure.adapter.out.persistence.repository;

import com.sprintforge.identity.auth.infrastructure.adapter.out.persistence.entity.AuthSessionEntity;
import org.jspecify.annotations.NullMarked;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.JpaSpecificationExecutor;

import java.util.Optional;
import java.util.UUID;

@NullMarked
public interface AuthSessionJpaRepository extends
        JpaRepository<AuthSessionEntity, UUID>,
        JpaSpecificationExecutor<AuthSessionEntity> {
    Optional<AuthSessionEntity> findById(UUID id);

    Optional<AuthSessionEntity> findByRefreshToken(String refreshToken);
}
