...
public class PermissionSpecs {
    ...
    public Specification<PermissionEntity> nameOrDescriptionLike(String searchTerm) {
        ...
        return (root, ignored, cb) -> cb.or(cb.like(cb.lower(root.get("name")), pattern), cb.like(cb.lower(root.get("description")), pattern));
    }

    public Specification<PermissionEntity> withFilters(@Nullable String searchTerm) {
        Specification<PermissionEntity> spec = (ignoredRoot, ignoredQuery, cb) -> cb.conjunction();
